...
import { 
  ...
  Calendar, Paperclip, LayoutGrid, Users, CheckCircle2, 
  Lock, X, Download, FileText, Clock, Edit2, Save, ExternalLink,
  Search, AlertCircle, ChevronDown, CheckSquare, Square
} from 'lucide-react';
import logo from './assets/logo.png'; 
...
export default function App() {
  const [token, setToken] = useState(localStorage.getItem('token'));
  const [user, setUser] = useState(JSON.parse(localStorage.getItem('user')) || null);
  const [activeView, setActiveView] = useState('board'); // 'board' | 'team-list'
...
function BoardView({ currentUser, activeTeam, filter }) {
  ...
  return (
    <div className="h-full flex flex-col p-8 overflow-x-auto">
      <header className="flex justify-between items-center mb-6">
        ...
        <div className="flex-1 max-w-md mx-6">
            <div className="relative">
                ...
                <input 
                    type="text" 
                    placeholder="Search by Title or Ticket ID (BB-XXXX)..." 
                    className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-slate-200 focus:ring-2 focus:ring-yellow-400 outline-none"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                />
      ...
      {isTaskModalOpen && <CreateTaskModal onClose={() => setIsTaskModalOpen(false)} onSuccess={fetchTasks} currentUser={currentUser} users={users} teams={teams} activeTeam={activeTeam} />}
      {isTeamModalOpen && <CreateTeamModal onClose={() => {setIsTeamModalOpen(false); window.location.reload();}} onSuccess={() => alert("Team Created!")} />}
      {selectedTask && <TaskDetailModal task={selectedTask} users={users} teams={teams} onClose={() => setSelectedTask(null)} onUpdate={fetchTasks} />}
    </div>
  );
}

// --- TASK DETAIL MODAL ---
function TaskDetailModal({ task, onClose, onUpdate, users, teams }) {
  const [loading, setLoading] = useState(false);
  ...
  const handleSave = async () => {
    // Validation
    if (!editForm.assigneeId || !editForm.reporterId || !editForm.startDate || !editForm.deadline || !editForm.teamId) {
  ...
  const addSubtask = () => {
      ...
      const newSub = { title: subtaskInput, isCompleted: false };
      ...
      <div className="bg-white w-full max-w-5xl rounded-lg shadow-2xl overflow-hidden animate-in zoom-in duration-200 h-[85vh] flex flex-col">
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex-1 p-8 overflow-y-auto custom-scrollbar border-r border-slate-100">
                 ...
                 {/* SUBTASKS */}
                 <div className="mb-8">
                     <label className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase mb-3"><CheckCircle2 size={14} /> Subtasks</label>
                     <div className="space-y-2 mb-3">
                         {editForm.subtasks.map((sub, idx) => (
                             <div key={idx} className="flex items-center gap-3 p-2 border border-slate-100 rounded bg-slate-50">
                                 <button onClick={() => toggleSubtask(idx)} className={sub.isCompleted ? "text-green-500" : "text-slate-300"}>
                                     {sub.isCompleted ? <CheckSquare size={18} /> : <Square size={18} />}
                                 </button>
                                 <span className={`text-sm ${sub.isCompleted ? 'line-through text-slate-400' : 'text-slate-700'}`}>{sub.title}</span>
                             </div>
                         ))}
                     </div>
                     <div className="flex gap-2">
                         ...
                         />
                         <button onClick={addSubtask} className="px-3 py-1.5 bg-slate-100 text-slate-600 text-xs font-bold rounded hover:bg-slate-200">Add</button>
                     </div>
                 </div>
      ...
      {(provided, snapshot) => (
        <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} onClick={onClick} className={`bg-white p-4 rounded-lg border border-slate-200 mb-3 shadow-sm hover:shadow-md transition group relative cursor-pointer ${snapshot.isDragging ? 'rotate-2 shadow-xl ring-2 ring-yellow-400 z-50' : ''}`}>
          ...
          <div className="flex justify-between items-center pt-3 border-t border-slate-50">
            ...
            <div className="flex items-center gap-2">
                {task.subtasks?.length > 0 && <div className="flex items-center gap-1 text-[10px] text-slate-500"><CheckCircle2 size={12}/> {task.subtasks.filter(s=>s.isCompleted).length}/{task.subtasks.length}</div>}
                {task.deadline && <div className={`flex items-center gap-1 text-[10px] font-medium ${new Date(task.deadline) < new Date() ? 'text-red-500' : 'text-slate-400'}`}><Calendar size={12} />{new Date(task.deadline).toLocaleDateString(undefined, {month:'short', day:'numeric'})}</div>}
            </div>
          </div>
        </div>
...
// --- CREATE TASK MODAL (Enforced Mandatory Fields) ---
function CreateTaskModal({ onClose, onSuccess, currentUser, users, teams, activeTeam }) {
  const [loading, setLoading] = useState(false);
  const [subtaskInput, setSubtaskInput] = useState('');
  const [formData, setFormData] = useState({
    title: '', description: '', 
  ...
  const addSubtask = () => {
      if(!subtaskInput.trim()) return;
      setFormData({...formData, subtasks: [...formData.subtasks, { title: subtaskInput, isCompleted: false }]});
      setSubtaskInput('');
  };

  const handleSubmit = async (e) => {
    e.preventDefault(); 
    if(!formData.teamId || !formData.assigneeId || !formData.reporterId || !formData.deadline || !formData.title || !formData.description) {
    ...
    Object.keys(formData).forEach(key => {
        if(key === 'files') { for(let i=0; i<formData.files.length; i++) data.append('files', formData.files[i]); } 
        else if (key === 'subtasks') { data.append('subtasks', JSON.stringify(formData.subtasks)); }
        else { data.append(key, formData[key]); }
    });
      ...
      <div className="bg-white w-full max-w-5xl rounded-lg shadow-2xl overflow-hidden animate-in zoom-in duration-200 h-[80vh] flex flex-col">
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex-1 p-8 overflow-y-auto custom-scrollbar border-r border-slate-100">
                 ...
                 <div className="mb-6">
                     <label className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase mb-2">Subtasks</label>
                     <div className="flex gap-2 mb-2">
                         <input className="flex-1 border border-slate-200 rounded px-3 py-1.5 text-sm" placeholder="Add subtask..." value={subtaskInput} onChange={e => setSubtaskInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addSubtask()} />
                         <button onClick={addSubtask} className="px-3 py-1.5 bg-slate-100 font-bold text-xs rounded">Add</button>
                     </div>
                     <ul className="space-y-1">
                         {formData.subtasks.map((s, i) => <li key={i} className="text-sm text-slate-600 flex items-center gap-2"><Square size={14}/> {s.title}</li>)}
                     </ul>
                 </div>
            ...
            <div className="w-80 bg-slate-50 p-6 overflow-y-auto custom-scrollbar space-y-6">
                ...
                <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Team <span className="text-red-500">*</span></label>
                    <select className="w-full bg-white border border-slate-200 rounded p-2 text-sm" value={formData.teamId} onChange={e => setFormData({...formData, teamId: e.target.value})}>
                        <option value="">Select Team</option>
                        {teams.map(t => <option key={t._id} value={t._id}>{t.name}</option>)}
                    </select>
                </div>
    ...
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shadow-xl z-20">
      ...
      <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
        ...
        <div className="mt-6">
            ...
            {teams.length === 0 && <div className="px-3 text-xs text-slate-400 italic">No teams joined.</div>}
            {teams.map(team => (
                <button 
                    key={team._id} 
                    onClick={() => { setActiveView('board'); setActiveTeam(team); }} 
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all mb-1 ${activeTeam?._id === team._id ? 'bg-blue-50 text-blue-700 font-bold' : 'text-slate-600 hover:bg-slate-50'}`}
                >
...
// (TeamView, AuthScreen, CreateTeamModal components remain mostly same but ensured they work with new structure)
function CreateTeamModal({ onClose, onSuccess }) {
  const [name, setName] = useState('');
  const [isPrivate, setIsPrivate] = useState(false);
  const [selectedMembers, setSelectedMembers] = useState([]);
...
function AuthScreen({ setToken, setUser }) {
  ...
  return (
    <div className="h-screen w-full flex items-center justify-center bg-slate-50">
      <div className="w-full max-w-md p-10 bg-white rounded-3xl shadow-2xl border border-slate-100">
        <div className="text-center mb-8"><h1 className="text-4xl font-black text-slate-900 mb-2">BeeBark</h1><p className="text-slate-500">Agile project management for high-performance teams.</p></div>
        <form onSubmit={handleSubmit} className="space-y-4">
           <div><label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Username</label><input className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-yellow-400 transition" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} required /></div>
           {!isLogin && <div><label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Email</label><input type="email" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-yellow-400 transition" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} required /></div>}
           <div><label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Password</label><input type="password" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-yellow-400 transition" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} required /></div>
